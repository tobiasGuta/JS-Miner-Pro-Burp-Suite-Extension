...
public class JsMinerExtension implements BurpExtension, HttpHandler {
    ...
    public ResponseReceivedAction handleHttpResponseReceived(HttpResponseReceived responseReceived) {
        ...
        api.logging().logToOutput("--- JS MINER: SCANNER TRIGGERED FOR URL: " + url + " ---");

        updateNoisePatterns();

        // 1. Scope Check
        if (configTab.isScopeOnly() && !api.scope().isInScope(url)) {
            api.logging().logToOutput("SKIPPING (Out of Scope): " + url);
            return ResponseReceivedAction.continueWith(responseReceived);
        }

        // 2. MIME Type Check
        String[] allowedMimes = configTab.getMimeTypes();
        // DEBUG: Log the exact list of MIME types being used for the check.
        api.logging().logToOutput("DEBUG: Allowed MIME types list: " + Arrays.toString(allowedMimes));
        ...
        if (!isMimeAllowed) {
            api.logging().logToOutput("SKIPPING (MIME Type not allowed: " + mime + "): " + url);
            return ResponseReceivedAction.continueWith(responseReceived);
        }

        api.logging().logToOutput("ANALYZING (MIME: " + mime + "): " + url);
