...
public class ConfigTab extends JPanel {
    ...
    public ConfigTab(MontoyaApi api) {
        ...
        resetButton.addActionListener(e -> {
            if (JOptionPane.showConfirmDialog(this, "This will reset rules and filters to their defaults. Continue?", "Confirm Reset", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                // **THE FIX IS HERE**: Explicitly reset all fields to their defaults before saving.
                createDefaultRules(); // Resets regex rules list
                mimeTypesTextArea.setText(DEFAULT_MIME_TYPES); // Resets MIME types text
                noisePatternsTextArea.setText(DEFAULT_NOISE_PATTERNS); // Resets noise patterns text
                inScopeOnlyCheckbox.setSelected(true); // Resets scope checkbox
                refreshTable();
                saveConfig();
            }
        });
    ...
    private void createDefaultRules() {
        rules = new ArrayList<>();
        // SECRETS
        rules.add(new RegexRule(true, "Generic High-Entropy Secret", "(?i)(?:secret|token|password|auth|key)['\"]?\\s*[:=]\\s*['\"]?([A-Za-z0-9\\-_:/.+=]{20,})['\"]?", "SECRET"));
        ...
        // ENDPOINTS & PATHS
        rules.add(new RegexRule(true, "Generic Path", "['\"](/[a-zA-Z0-9/._-]{8,})['\"]", "ENDPOINT"));
